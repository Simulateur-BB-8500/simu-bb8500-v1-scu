...
static void _GPIO_set_mode(const GPIO* gpio, GPIO_mode_t mode) {
	...
	(gpio -> port) -> MODER |= (0b11 << (2 * (gpio -> pin))); // MODERy = '11'.
	// Set required bits.
	switch(mode) {
	case GPIO_MODE_INPUT:
		// MODERy = '00'.
		(gpio -> port) -> MODER &= ~(0b11 << (2 * (gpio -> pin)));
		break;
	case GPIO_MODE_OUTPUT:
		// MODERy = '01'.
		(gpio -> port) -> MODER &= ~(0b1 << (2 * (gpio -> pin) + 1));
		break;
	case GPIO_MODE_ALTERNATE_FUNCTION:
		// MODERy = '10'.
		(gpio -> port) -> MODER &= ~(0b1 << (2 * (gpio -> pin)));
		break;
...
static void _GPIO_set_output_speed(const GPIO* gpio, GPIO_output_speed_t output_speed) {
	...
	(gpio -> port) -> OSPEEDR &= ~(0b11 << (2 * (gpio -> pin)));
	// Set required bits.
	switch(output_speed) {
	case GPIO_SPEED_LOW:
		// Nothing to do.
	...
	case GPIO_SPEED_MEDIUM:
		// OSPEEDRy = '01'.
		(gpio -> port) -> OSPEEDR |= (0b1 << (2 * (gpio -> pin)));
		break;
	case GPIO_SPEED_HIGH:
		// OSPEEDRy = '10'.
		(gpio -> port) -> OSPEEDR |= (0b1 << (2 * (gpio -> pin) + 1));
		break;
...
static void _GPIO_set_pull_resistor(const GPIO* gpio, GPIO_pull_resistor_t pull_resistor) {
	...
	(gpio -> port) -> PUPDR &= ~(0b11 << (2 * (gpio -> pin)));
	// Set required bits.
	...
	case GPIO_PULL_UP:
		// PUPDRy = '01'.
		(gpio -> port) -> PUPDR |= (0b1 << (2 * (gpio -> pin)));
		break;
	case GPIO_PULL_DOWN:
		// PUPDRy = '10'.
		(gpio -> port) -> PUPDR |= (0b1 << (2 * (gpio -> pin) + 1));
		break;
	default:
		break;
	}
 ...
 * @return: 			None.
 */
static void _GPIO_set_alternate_function(const GPIO* gpio, uint32_t alternate_function) {
	// Clamp AF number.
	...
	if ((gpio -> pin) < GPIO_AFRH_OFFSET) {
		...
		(gpio -> port) -> AFRL &= ~(0b1111 << (4 * (gpio -> pin)));
		(gpio -> port) -> AFRL |= (alternate_function << (4 * (gpio -> pin)));
	}
	else {
		// Set AFRH register: AFRy = 'alternate_function'.
		(gpio -> port) -> AFRH &= ~(0b1111 << (4 * ((gpio -> pin) - GPIO_AFRH_OFFSET)));
		(gpio -> port) -> AFRH |= (alternate_function << (4 * ((gpio -> pin) - GPIO_AFRH_OFFSET)));
	}
}
 ...
 * @param output_speed: 	Output speed (see enum defined in gpio.h).
 * @param pull_resistor:	Resistor configuration (see enum defined in gpio.h).
 * @param alternate_function: 		Alternate function number (0 to 15) if 'GPIO_MODE_ALTERNATE_FUNCTION' mode is selected.
 */
 ...
 * @return: 		None.
 */
void __attribute__((optimize("-O0"))) GPIO_write(const GPIO* gpio, uint8_t state) {
	// Set bit.
	if (state == 0) {
 ...
 */
uint8_t __attribute__((optimize("-O0"))) GPIO_read(const GPIO* gpio) {
	// Check mode.
	uint8_t state = 0;
