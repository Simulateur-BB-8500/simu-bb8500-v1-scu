...
void NVIC_EnableInterrupt(InterruptVector it_num) {
	// (it_num >> 5) selects the proper register (ISER0 to ISER7) acting as a modulo.
	// (it_num & 0x0000001F) selects the proper bit to set.
	NVIC -> ISER[it_num >> 5] = (0b1 << (it_num & 0x0000001F));
}
...
void NVIC_DisableInterrupt(InterruptVector it_num) {
	// (it_num >> 5) selects the proper register (ICER0 to ICER7) acting as a modulo.
	// (it_num & 0x0000001F) selects the proper bit to set.
	NVIC -> ICER[it_num >> 5] = (0b1 << (it_num & 0x0000001F));
}
