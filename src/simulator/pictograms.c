...
void PICTOGRAMS_Task(void) {
	...
	case PICTOGRAMS_STATE_OFF:
		...
		if (lsmcu_ctx.zba_closed != 0) {
			...
			// Compute next state.
			pictograms_ctx.switch_state_time = TIM2_GetMs();
	...
	case PICTOGRAMS_STATE_ZBA_CLOSED_TRANSITION1:
		if (lsmcu_ctx.zba_closed == 0) {
			...
			if (TIM2_GetMs() > (pictograms_ctx.switch_state_time + PICTOGRAMS_ZBA_CLOSED_LSGR_DELAY_MS)) {
	...
	case PICTOGRAMS_STATE_ZBA_CLOSED_TRANSITION2:
		if (lsmcu_ctx.zba_closed == 0) {
			...
			if (TIM2_GetMs() > (pictograms_ctx.switch_state_time + PICTOGRAMS_ZBA_CLOSED_LSBA_BLINK_DURATION_MS)) {
				// Turn LSBA off.
	...
	case PICTOGRAMS_STATE_DJ_LOCKED_TRANSITION1:
		...
		if (TIM2_GetMs() > (pictograms_ctx.switch_state_time + PICTOGRAMS_ZDJ_LOCKING_DURATION_MS)) {
	...
	case PICTOGRAMS_STATE_DJ_LOCKED_TRANSITION2:
		// Wait for LSDJ delay.
		if (TIM2_GetMs() > (pictograms_ctx.switch_state_time + PICTOGRAMS_ZDJ_LOCKING_LSDJ_DELAY_MS)) {
			// Turn LSDJ off.
	...
	case PICTOGRAMS_STATE_DJ_LOCKED:
		...
		if (lsmcu_ctx.rheostat_0 == 0) {
			if ((lsmcu_ctx.lsrh_blink_request != 0) && (GPIO_Read(&GPIO_LSRH) != 0)) {
				...
				pictograms_ctx.lsrh_blink_start_time = TIM2_GetMs();
			}
			else {
				if (TIM2_GetMs() > (pictograms_ctx.lsrh_blink_start_time + PICTOGRAMS_LSRH_BLINK_DURATION_MS)) {
					// End blink.
